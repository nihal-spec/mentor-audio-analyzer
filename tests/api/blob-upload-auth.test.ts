...
describe('POST /api/blob-upload — authorization endpoint', () => {
  it('returns a clientToken for a valid pathname in payload', async () => {
    // The @vercel/blob/client SDK sends { type, payload: { pathname } }
    const res = await postJson({ type: 'blob.generate-client-token', payload: { pathname: 'analysis/1234-session.mp3' } });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.clientToken).toBe('mock-client-token-abc123');
  });

  it('rejects requests missing pathname in payload', async () => {
    const res = await postJson({ type: 'blob.generate-client-token', payload: {} });
  ...
  it('accepts pathnames with special characters (sanitized downstream)', async () => {
    const res = await postJson({ type: 'blob.generate-client-token', payload: { pathname: 'analysis/2026-09-17_tutoring.wav' } });
    expect(res.status).toBe(200);
    const json = await res.json();
